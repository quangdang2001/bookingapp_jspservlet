...
public class RoomServiceIplm implements RoomService {
    ...
    @Override
    public List<Room> filterRoom(String roomType, List<Integer> star, Double lowPrice, Double maxPrice,List<Room> rooms) {
        if (roomType!= null){
            rooms.removeIf(room -> !room.getRoomType().getName().equals(roomType));
        }
        if (star!=null){
